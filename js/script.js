var tab_list = []; // store all programming languages and frameworks.
var project_list = {}; // Store project data 
var total_project = 0; // Total project count
var project_counter = 0; // Project counter for particular languages and frameworks.

// fetch readme.md of given language and extract & return project details as list[objects] 
function fetchAllProjectsFromLanguage(language) {
  return fetch("https://raw.githubusercontent.com/Nisheet-Patel/Mini-Projects/master/" + language + "/README.md")
    .then((response) => {
      if (!response.ok) {
...
// fetch markdown and extract all programming languages and frameworks.
fetch("https://raw.githubusercontent.com/Nisheet-Patel/Mini-Projects/master/README.md")
  .then((response) => {
    if (!response.ok) {
      throw new Error("Network response was not ok");
    }
  ...
  .finally(() => {
    ...
    tab_list.forEach(function (language) {
      if (language != "All") {
        fetchAllProjectsFromLanguage(language)
          .then((data) => {
            project_list[language] = data;
          })
          .catch((error) => {
...
// Render project cards for the specified language in the UI
function renderProjectCards(language) {
  project_counter = 0;
  ...
  displayTotalProjectCount(project_counter);
}
// Render project cards for all languages in the UI.
function renderAllProjectCards() {
  ...
  for (const language in project_list) {
    ...
    total_project += project_counter;
  }

  displayTotalProjectCount(total_project);
}
  ...
  .addEventListener("click", function (event) {
    ...
    if (event.target.classList.contains("tab")) {
      ...
      if (event.target.textContent != "All") {
        ...
        renderProjectCards(event.target.textContent);
      } else {
        clearProjectContainer();
        renderAllProjectCards();
      }
...
// Update the total project count element with the provided value.
function displayTotalProjectCount(value) {
  document.querySelector("#total_project").innerHTML = value;
}

// Remove all project cards from the project container.
